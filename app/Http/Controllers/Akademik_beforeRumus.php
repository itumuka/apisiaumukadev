...
class Akademik extends Controller
{
    ...
    {
        $id = $request->id_kelas;
        return (new TemplateUASExport($id))->download('Template_Input_Nilai_UAS.xlsx');
    ...
    public function import_nilai_uas(Request $request)
    {
        ...
        if ($request->hasFile('fileimport')) {
            ...
            // Menggunakan instance import dengan validasi error
            $import = new NilaiUASImport();
            Excel::import($import, $file);
    
            // Ambil error yang terjadi selama import
            $errors = $import->getErrors();
    
            // Jika ada error, kembalikan sebagai respons JSON
            if (!empty($errors)) {
            ...
            // Jika tidak ada error, beri respon sukses
            return response()->json([
                'status' => 'success',
    ...
    {
        $data_dosen = $this->akademik->dosen();
        return $data_dosen;
    }
    ...
    public function saveAllQrCode(Request $request)
    {
        ...
        $validatedData = $request->validate([
            ...
            'qr_data.*.nidn' => 'required|string',
            'qr_data.*.qr_code' => 'required|string'
        ]);

        foreach ($validatedData['qr_data'] as $qrItem) {
            // Decode Base64 QR Code Image
            $qrData = $qrItem['qr_code'];
            ...
            $filePath = 'qr_codes/' . $qrItem['nidn'] . '.png';
            $namafile = $qrItem['nidn'] . '.png';
        ...
        return response()->json(['success' => 'Semua QR Code berhasil disimpan!']);
    }
