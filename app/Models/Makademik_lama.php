...
class Makademik extends Model
{
    ...
    {
        $getmhs_cetak = collect(DB::select("SELECT a.nim,a.nama_mahasiswa,b.nama_program_studi, b.kode_program_studi, b.nama_program_studi,b.kode_jenjang_pendidikan, nama_fakultas, tahun_angkatan, CONCAT_WS(' ', ds.gelar_depan, ds.nama, ds.gelar_belakang) AS dosen_wali,dk.nama as dekane,(SELECT CONCAT(nama) FROM simpeg_pegawai WHERE id=b.pimpinan_prodi) AS namaprodi 
    ...
    {
        $getdaftarhadirujian_cetak = DB::select("SELECT *,CONCAT_WS(' ', e.gelar_depan, e.nama,e.gelar_belakang) AS nama_dosen,CONCAT_WS(' ', h.gelar_depan, h.nama,h.gelar_belakang) AS nama_dosen2,(SELECT nama FROM simpeg_pegawai aa WHERE g.pimpinan=aa.id) AS dekane FROM akd_kelas_kuliah a JOIN akd_penawaran_matakuliah b ON a.id_tawar=b.id_tawar
