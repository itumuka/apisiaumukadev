...
class Makademik extends Model
{
    ...
    public function cek_transkrip_krs(Request $request)
    {
        ...
        (
        SELECT (akd_matakuliah.sks_matakuliah*mutu) AS kum_sksmutu
                FROM akd_transkrip
                JOIN akd_matakuliah ON akd_matakuliah.id_matakuliah = akd_transkrip.id_matakuliah 
                JOIN akd_predikat_nilai_huruf ON akd_transkrip.nilai = akd_predikat_nilai_huruf.nilai_huruf_akhir 
                WHERE akd_transkrip.nim ='" . $request->nim . "' AND akd_transkrip.id_matakuliah = mk.id_matakuliah
    ...
    {
        $list_ba = DB::select("SELECT a.*,b.hadir, DATE_FORMAT(a.tgl,'%d-%m-%Y') AS tgl_indo  FROM akd_berita_acara a LEFT JOIN akd_presensi_mhs b ON a.id_kelas=b.id_kelas AND a.pertemuan_ke=b.pertemuan WHERE a.id_kelas = '$request->id_kelas' ORDER BY a.pertemuan_ke ASC");
        return $list_ba;
    }
    ...
    public function cetakkrs(Request $request)
    {
        ...
        CONCAT_WS(' ', gelar_depan, simpeg_pegawai.nama,gelar_belakang) AS nama_dosen FROM akd_heregistrasi
    ...
    public function cetakkhs(Request $request)
    {
        ...
        //akd_heregistrasi JOIN akd_krs ON akd_heregistrasi.id_heregistrasi=akd_krs.id_heregistrasi
        ...
        (akd_predikat_nilai_huruf.mutu * akd_matakuliah.sks_matakuliah) AS total_nilai, akd_penawaran_matakuliah.kode_dosen, CONCAT_WS(' ', gelar_depan, simpeg_pegawai.nama,gelar_belakang) AS nama_dosen
        FROM akd_krs
    ...
    {
        $getmhs_cetak = collect(DB::select("SELECT a.nim,a.nama_mahasiswa,b.nama_program_studi, b.kode_program_studi, b.nama_program_studi, nama_fakultas, tahun_angkatan, CONCAT_WS(' ', gelar_depan, ds.nama, gelar_belakang) AS dosen_wali,(SELECT CONCAT(gelar_depan,' ',nama,' ',gelar_belakang) FROM simpeg_pegawai WHERE id=b.pimpinan_prodi) AS namaprodi 
    ...
    {
        $daftar_mahasiswa = DB::select("SELECT a.id_detail_krs,e.nim, e.nama_mahasiswa, g.nama_matakuliah, nilai_uts, a.nilai_akhir_angka,a.nilai_akhir_huruf, f.sks_matakuliah, f.smt_matakuliah, f.tahun_kurikulum, f.id_matakuliah FROM akd_detail_krs a 
        JOIN akd_krs b ON a.id_krs=b.id_krs 
        JOIN akd_heregistrasi c ON b.id_heregistrasi = c.id_heregistrasi
        JOIN akd_kelas_kuliah d ON a.id_kelas = d.id_kelas
        JOIN akd_mahasiswa e ON c.nim = e.nim
        ...
        WHERE a.id_kelas='" . $request->id_kelas . "' ORDER BY e.nim");
    ...
    public function data_hitung_presensi(Request $request)
    {
        ...
        $absensi = DB::select("SELECT * FROM akd_presensi_mhs WHERE id_kelas = '" . $request->id_kelas . "'");
        return $absensi;
    }
    ...
    public function data_makulpenawaran(Request $request)
    {
        ...
        if ($request->tipe == 'Dekanat') {
            ...
            $makulpenawaran = DB::select("SELECT akd_penawaran_matakuliah.*, akd_matakuliah.*, akd_kelas_kuliah.*, akd_program_studi.*,
            CONCAT_WS('-',DATE_FORMAT(jam_mulai,'%H:%i'), DATE_FORMAT(jam_selesai,'%H:%i')) AS waktu, dosen1.id AS id_dosen1,dosen1.nidn AS nidn_dosen1,dosen1.nama AS nama_dosen1,dosen1.gelar_depan AS gelar_depan_dosen1,
    ...
    public function hapus_makulpenawaran(Request $request)
    {
        ...
        $hapus = DB::table('akd_kelas')->where('id_tawar', $request->id)->delete();
        return $hapusmakulprasyarat;
    }
    ...
    {
        $tampiltahunangkatan = DB::select("SELECT tahun_angkatan FROM akd_mahasiswa WHERE lulus='0' group by tahun_angkatan order by tahun_angkatan desc");
    ...
    public function dosen()
    {
        ...
        LEFT JOIN akd_fakultas ON akd_fakultas.kode_fakultas = akd_program_studi.kode_fakultas
        INNER JOIN simpeg_jenis ON simpeg_jenis.id = simpeg_pegawai.kode_jenis
    ...
    {
        $thn = $request->tahunangkatan;
        $mahasiswa = DB::select("SELECT *,a.kode_agama AS agama_mhs,b.kode_provinsi AS provinsi_mhs,b.kode_kabupaten AS kabupaten_mhs,b.rt AS rt_mhs,b.rw AS rw_mhs,b.kode_pos AS kode_pos_mhs,b.alamat_asal AS alamat_mhs,n.id AS status_pernikahan,h.nama AS nama_ayah,i.nama AS nama_ibu,h.rt AS rt_ayah,h.rw AS rw_ayah,h.kode_pos AS kode_pos_ayah,h.alamat AS alamat_ayah,l.pekerjaan_singkatan AS pekerjaan_ayah
        ,i.rt AS rt_ibu,i.rw AS rw_ibu,h.kode_agama AS agama_ayah,i.kode_agama AS agama_ibu,h.kode_jenjang_pendidikan AS jenjangpendidikan_ayah,i.kode_jenjang_pendidikan AS jenjangpendidikan_ibu,
        i.kode_pos AS kode_pos_ibu,i.alamat AS alamat_ibu,m.pekerjaan_singkatan AS pekerjaan_ibu,h.kode_jenis_pekerjaan AS kode_pekerjaan_ayah,i.kode_jenis_pekerjaan AS kode_pekerjaan_ibu,h.kode_penghasilan AS kode_penghasilan_ayah,i.kode_penghasilan AS kode_penghasilan_ibu,h.alamat AS alamat_ayah,i.alamat AS alamat_ibu FROM akd_mahasiswa a 
        ...
        LEFT JOIN mst_pekerjaan l ON h.kode_jenis_pekerjaan=l.pekerjaan_id 
        LEFT JOIN mst_pekerjaan m ON i.kode_jenis_pekerjaan=m.pekerjaan_id 
        LEFT JOIN mst_status_nikah n ON a.status_nikah=n.id WHERE a.tahun_angkatan LIKE '%$thn%' ORDER BY a.tahun_angkatan DESC -- and lulus='0'");
        return $mahasiswa;
    }
    // Password Mahasiswa
    ...
    public function kartuujian(Request $request)
    {
        ...
        $kartuujian = DB::select("SELECT * FROM akd_mahasiswa a 
        ...
        JOIN akd_program_studi d ON d.kode_program_studi=a.kode_program_studi WHERE b.tahun LIKE '%$tahun%' AND b.semester='$semester' AND a.tahun_angkatan LIKE '%$thn%'
         AND b.krs='1'");
        return $kartuujian;
    }
    ...
    {
        $thn = $request->tahunangkatan;
        $daftarmaba = DB::select("SELECT * FROM adm_camaba JOIN akd_program_pendidikan ON adm_camaba.kode_program_pendidikan=akd_program_pendidikan.kode_program_pendidikan JOIN akd_program_studi ON adm_camaba.kode_program_studi=akd_program_studi.kode_program_studi JOIN adm_jalur_pmb ON adm_camaba.kode_jalur_pmb=adm_jalur_pmb.kode_jalur_pmb where adm_camaba.tahun LIKE '%$thn%' 
        ");
        return $daftarmaba;
    }
    // Mahasiswa Lulusan 1
    ...
    public function detail_camaba(Request $request)
    {
        ...
        $detail_camaba = DB::select("SELECT * FROM adm_camaba JOIN akd_program_pendidikan ON adm_camaba.kode_program_pendidikan=akd_program_pendidikan.kode_program_pendidikan JOIN akd_program_studi ON adm_camaba.kode_program_studi=akd_program_studi.kode_program_studi JOIN adm_jalur_pmb ON adm_camaba.kode_jalur_pmb=adm_jalur_pmb.kode_jalur_pmb where adm_camaba.id_camaba='$id_camaba'");
    ...
    public function simpan_camaba(Request $request)
    {
        ...
        $simpanayah = DB::table('adm_ortu_ayah')->insert([
            'no_pendaftaran'  =>  $no_pendaftaran,
            ...
            'kode_jenis_pekerjaan'  =>  $request->pekerjaan_ayah,
            'kode_jenjang_pendidikan'  =>  $request->pendidikan_ayah,
            'kode_penghasilan'  =>  $request->penghasilan,
            'telepon_ayah'  =>  $request->no_telepon
        ]);
        $simpanibu = DB::table('adm_ortu_ibu')->insert([
            'no_pendaftaran'  =>  $no_pendaftaran,
            ...
            'kode_jenis_pekerjaan'  =>  $request->pekerjaan_ibu,
            'kode_jenjang_pendidikan'  =>  $request->pendidikan_ibu,
            'kode_penghasilan'  =>  $request->penghasilan,
    ...
    public function edit_mahasiswa(Request $request)
    {
        $editmahasiswa = DB::table('akd_mahasiswa')
            ...
            ->update([
                'kode_jalur_pmb'  =>  $request->jalur_pmb11,
                // 'kode_program_studi'  =>  $request->nama_program_studi,
                'kode_fakultas'  =>  $request->nama_fakultas11,
                'tempat_lahir'  =>  $request->tempat_lahir11,
                'tanggal_lahir'  =>  $request->tanggal_lahir11,
                'nama_mahasiswa'  =>  $request->nama_camaba11,
                'alamat_asal'  =>  $request->alamat_asal11,
                'kode_agama'  =>  $request->nama_agama11,
                'status_nikah'  =>  $request->status_nikah11,
                'jenis_kelamin'  =>  $request->jenis_kelamin11
            ]);
        DB::table('adm_camaba')
            ...
            ->update([
                ...
                'kode_jalur_pmb'  =>  $request->jalur_pmb11,
                // 'kode_program_studi'  =>  $request->nama_program_studi,
                'nama_camaba'  =>  $request->nama_camaba11,
                'tempat_lahir'  =>  $request->tempat_lahir11,
                'tanggal_lahir'  =>  $request->tanggal_lahir11,
                'jenis_kelamin'  =>  $request->jenis_kelamin11,
                'kode_agama'  =>  $request->nama_agama11,
        ...
        DB::table('adm_ortu_ayah')
            ...
            ->update([
                ...
                'kode_jenis_pekerjaan'  =>  $request->kode_jenis_pekerjaan_ayah11,
                'kode_jenjang_pendidikan'  =>  $request->jenjang_pendidikan_ayah11,
                'kode_penghasilan'  =>  $request->kode_penghasilan_ayah11,
                'telepon_ayah'  =>  $request->telepon_ayah11
            ]);

        DB::table('adm_ortu_ibu')
            ->where('no_pendaftaran', $request->no_pendaftaran11)
            ->update([
                'nama'  =>  $request->nama_ibu11,
                'alamat'  =>  $request->alamat_ibu11,
                // 'rt'  =>  $request->rt,
                // 'rw'  =>  $request->rw,
                // 'kode_kabupaten'  =>  $request->kabupatenortu,
                // 'kode_propinsi'  =>  $request->provinsiortu,
                ...
                'kode_jenis_pekerjaan'  =>  $request->kode_jenis_pekerjaan_ibu11,
                'kode_jenjang_pendidikan'  =>  $request->jenjang_pendidikan_ibu11,
    ...
    {
        $getdaftarhadirkuliah_cetak = DB::select("SELECT *,CONCAT_WS(' ', e.gelar_depan, e.nama,e.gelar_belakang) AS nama_dosen,CONCAT_WS(' ', h.gelar_depan, h.nama,h.gelar_belakang) AS nama_dosen2 FROM akd_kelas_kuliah a JOIN akd_penawaran_matakuliah b ON a.id_tawar=b.id_tawar
        LEFT JOIN simpeg_pegawai e ON e.id=a.kode_dosen
        LEFT JOIN simpeg_pegawai h ON h.id=a.kode_dosen2
    ...
    {
        $getdaftarhadirujian_cetak = DB::select("SELECT *,CONCAT_WS(' ', e.gelar_depan, e.nama,e.gelar_belakang) AS nama_dosen,CONCAT_WS(' ', h.gelar_depan, h.nama,h.gelar_belakang) AS nama_dosen2 FROM akd_kelas_kuliah a JOIN akd_penawaran_matakuliah b ON a.id_tawar=b.id_tawar
               JOIN simpeg_pegawai e ON e.id=a.kode_dosen
               LEFT JOIN simpeg_pegawai h ON a.kode_dosen2=h.id
               JOIN akd_program_studi d ON b.kode_program_studi=d.kode_program_studi
               JOIN akd_matakuliah f ON b.id_matakuliah=f.id_matakuliah
               JOIN akd_fakultas g ON d.kode_fakultas=g.kode_fakultas
        WHERE a.id_tawar ='" . $request->id_tawar . "'");
    ...
    {
        $getkartuujian_cetak = DB::select("SELECT *,CONCAT_WS(' ', gelar_depan, ds.nama, gelar_belakang) AS dosen_wali FROM akd_mahasiswa a JOIN akd_heregistrasi b ON a.nim=b.nim
        JOIN akd_krs c ON c.id_heregistrasi=b.id_heregistrasi
        JOIN akd_detail_krs d ON d.id_krs=c.id_krs
        JOIN akd_kelas_kuliah e ON e.id_kelas=d.id_kelas
        JOIN akd_penawaran_matakuliah f ON f.id_tawar=e.id_tawar
    ...
    {
        $kartuhasilstudi = DB::select("SELECT *,CONCAT_WS(' ', i.gelar_depan, i.nama,i.gelar_belakang) AS nama_dosen,CONCAT_WS(' ', h.gelar_depan, h.nama,h.gelar_belakang) AS nama_dosen2 FROM akd_mahasiswa a JOIN akd_heregistrasi b ON a.nim=b.nim
        JOIN akd_krs c ON c.id_heregistrasi=b.id_heregistrasi
        JOIN akd_detail_krs d ON d.id_krs=c.id_krs
        JOIN akd_kelas_kuliah e ON e.id_kelas=d.id_kelas
        JOIN akd_penawaran_matakuliah f ON f.id_tawar=e.id_tawar
...
FROM akd_transkrip,akd_matakuliah,akd_predikat_nilai_huruf WHERE akd_transkrip.nim='" . $request->nim . "' AND akd_transkrip.id_matakuliah=akd_matakuliah.id_matakuliah
            AND akd_transkrip.nilai=akd_predikat_nilai_huruf.nilai_huruf_akhir GROUP BY akd_matakuliah.id_matakuliah ORDER BY akd_transkrip.id_matakuliah ASC
    ...
    {
        $gettranskipnilai_cetak = DB::select("SELECT akd_mahasiswa.*,adm_camaba.*,akd_program_studi.*,akd_fakultas.*,akd_kelengkapan_transkrip.*,CONCAT(gelar_depan,' ',simpeg_pegawai.nama,', ',gelar_belakang) AS namakaprodi,(SELECT CONCAT(gelar_depan,' ',nama,' ',gelar_belakang) FROM simpeg_pegawai WHERE id=akd_fakultas.pimpinan) AS namadekan,DATE_FORMAT(tanggal_lulus, '%d-%m-%Y') AS tgllulus,DATE_FORMAT(tanggal_yudicium, '%d-%m-%Y') AS tglyud,DATE_FORMAT(tgl_registrasi, '%d-%m-%Y') AS tglmasuk,DATE_FORMAT(akd_mahasiswa.tanggal_lahir, '%d-%m-%Y') AS tgllahir FROM akd_mahasiswa JOIN adm_camaba ON 
