...
class Mmahasiswa extends Model
{
    ...
    public function ambilkrs(Request $request)
    {
        ...
        $mhscek = collect(DB::select("SELECT * FROM akd_mahasiswa WHERE nim='" . $request->nim . "'"))->first();
    ...
    public function simpan_krs(Request $request)
    {
        ...
        if ($num_cek > 0) {
            ...
            foreach ($getSKS as $rSKS) {
                ...
                if ($rPenawaran->id_matakuliah == $rIDmatkul->id_matakuliah) {
                    return response()->json(['error' => 'Mohon maaf!! matakuliah ini telah diambil. Terima kasih']);
                }
                //$summary_SKS = $summary_SKS + $rPenawaran['sks_matakuliah'];
            }

            $getSKS = DB::select("SELECT
    ...
    {
        $nim = $request->nim;
        $sttpem = DB::select("SELECT a.id_tagihan,a.nama_biaya,a.tahun,a.semester,a.biaya,(SELECT IF(SUM(bayar) IS NULL,0,SUM(bayar)) AS bayar FROM keu_bayar kb WHERE a.id_tagihan=kb.id_tagihan) AS jumbayar,a.status,(SELECT nim FROM keu_virtual_akun WHERE kode=a.kode_biling) AS kodeva FROM keu_tagihan a 
        JOIN akd_mahasiswa b ON a.nim=b.nim JOIN akd_program_studi c  ON c.kode_program_studi=b.kode_program_studi 
        WHERE a.nim='$nim' ORDER BY a.id_tagihan DESC");
    ...
    public function upload_foto(Request $request)
    {
        ...
        if ($validation->fails()) {
            ...
            // $cekfoto = collect(DB::connection('mysql')->select('SELECT foto FROM akd_mahasiswa WHERE nim = "'.$request->nim.'"'))->first();

            // $path = config('setting.second_url').'/images/foto_mahasiswa/'.$cekfoto->foto;
            ...
            if ($request->hasfile('file_upload')) {
                ...
                $name = $request->nim . '.' . $file->getClientOriginalExtension();
                $file->move(config('multiurl.second_url') . '/images/foto_mahasiswa/', $name);
    ...
    public function tampilkecamatan(Request $request)
    {
        ...
        $kd_provinsi = $request->kd_provinsi;
        $tampilkabupaten = DB::select("select * from adm_kecamatan WHERE kd_kabupaten='$kd_kabupaten' AND kd_provinsi='$kd_provinsi'");
    ...
    public function checkedom(Request $request)
    {
        ...
        $getid_mhs = collect(DB::select("SELECT id_mhs FROM akd_mahasiswa WHERE nim ='" . $request->nim . "'"))->first();
