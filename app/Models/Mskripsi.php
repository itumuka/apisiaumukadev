...
class Mskripsi extends Model
{
    ...
    public function cekKelayakan($nim, $fase)
    {
        ...
        if ($syaratList->isEmpty() && $prodiConfig) {
            ...
            // 4. Course Grade Validation for Seminar Proposal
            if ($fase == 'sempro') {
                $has_sempro_grade = $this->checkSemproGrade($nim);
                if (!$has_sempro_grade) $semua_lolos = false;

                $hasil[] = [
                    'no' => $index++,
                    'id_syarat_prodi' => null,
                    'syarat' => 'Nilai Matakuliah Seminar Proposal',
                    'isi' => $has_sempro_grade ? 'Sudah Ada' : 'Belum Ada',
                    'hubungi' => 'Bagian Akademik',
                    'status' => $has_sempro_grade ? 'v' : 'x',
                    'jenis' => 'sistem',
                    'is_wajib' => 1,
                    'tipe_upload' => null,
                    'kode_syarat' => 'NILAI_SEMPRO'
                ];
            }

            // ACC Sempro Pembimbing requirement removed - students can register sempro directly
            if ($fase == 'sempro') {
                // Advisor approval is no longer required for sempro registration
            }
            ...
            // 7. Surat Keterangan Bebas Administrasi Keuangan (BAK)
            if ($fase == 'ujian') {
                $bebasKeu = DB::table('keu_bebas_administrasi')
                    ->where('nim', $nim)
                    ->where('keperluan', 'UJIAN_SKRIPSI')
        ...
        if ($fase == 'ujian') {
            ...
            if (!in_array('BEBAS_KEUANGAN_BAK', $kodeSyaratProdi)) {
                $bebasKeu = DB::table('keu_bebas_administrasi')
                    ->where('nim', $nim)
                    ->where('keperluan', 'UJIAN_SKRIPSI')
                    ->where('status', 'APPROVED')
    ...
    /**
     * Check if student has a grade in Seminar Proposal course
     */
    ...
    public function getMahasiswaBimbingan($id_dosen, $tahun = null, $semester = null)
    {
        $query = DB::table('akd_skripsi as s')
            ...
            ->select(
                ...
                DB::raw("(SELECT status FROM akd_skripsi_ujian u WHERE u.id_skripsi = s.id ORDER BY u.id DESC LIMIT 1) as status_ujian"),
                DB::raw("(SELECT status FROM akd_skripsi_proposal pr WHERE pr.id_skripsi = s.id ORDER BY pr.id DESC LIMIT 1) as status_sempro")
            )
