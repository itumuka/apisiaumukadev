...
GROUP BY akd_matakuliah.id_matakuliah ORDER BY akd_transkrip.id_matakuliah ASC
    ...
    {
        $gettranskipnilai_cetak = DB::select("SELECT akd_mahasiswa.*,adm_camaba.*,akd_program_studi.*,akd_fakultas.*,akd_kelengkapan_transkrip.*,CONCAT(gelar_depan,' ',simpeg_pegawai.nama,', ',gelar_belakang) AS namakaprodi,(SELECT CONCAT(gelar_depan,' ',nama,' ',gelar_belakang) FROM simpeg_pegawai WHERE id=akd_fakultas.pimpinan) AS namadekan,(SELECT COALESCE(nip, nidn, '-') FROM simpeg_pegawai WHERE id=akd_fakultas.pimpinan) AS nipdekan,DATE_FORMAT(tanggal_lulus, '%d-%m-%Y') AS tgllulus,DATE_FORMAT(tanggal_yudicium, '%d-%m-%Y') AS tglyud,DATE_FORMAT(tgl_registrasi, '%d-%m-%Y') AS tglmasuk,DATE_FORMAT(akd_mahasiswa.tanggal_lahir, '%d-%m-%Y') AS tgllahir FROM akd_mahasiswa JOIN adm_camaba ON 
