...
class Mdekanat extends Model
{
    ...
    public function data_acckrs(Request $request)
    {
        ...
        if ($request->dosen_wali == 1) {
            ...
            $data_acckrs = DB::select("SELECT akd_heregistrasi.id_heregistrasi, akd_krs.id_krs, akd_mahasiswa.nim,akd_mahasiswa.nama_mahasiswa, akd_krs.ip_kumulatif, akd_krs.batas_sks, akd_krs.sks_bayar, akd_krs.sks_ambil, akd_heregistrasi.krs, akd_program_studi.nama_program_studi, akd_heregistrasi.tahun, akd_heregistrasi.semester, IFNULL(akd_mahasiswa.telp,'-') AS no_hp, akd_heregistrasi.valid_id 
            FROM
            akd_heregistrasi
    ...
    {
        $daftarmhs_pa = DB::select("SELECT akd_mahasiswa.nim, nama_mahasiswa, tahun_angkatan,nama_agama,nama_program_pendidikan, nama_program_studi, akd_mahasiswa.telp AS no_hp, CONCAT_WS(' ', gelar_depan, simpeg_pegawai.nama,gelar_belakang) AS dosen_wali, IF(sks_ambil > 0, 'KRS','Tidak KRS') AS status_krs,tbl1.nim AS cekher,akd_mahasiswa.semester
        FROM akd_mahasiswa 
        LEFT JOIN adm_camaba ON adm_camaba.no_pendaftaran = akd_mahasiswa.no_pendaftaran
